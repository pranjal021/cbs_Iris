...
function commandHandlers(command) {
    ...
    } else if (command === 'iris_javascript'){
        python = false
        javascript = true
        console.log("Calling jsView")
        $('#editorspane').css('grid-template-columns', '100% 0% 0%');
    }else if (command === 'iris_python'){
        python = true
        javascript = false
        $('#editorspane').css('grid-template-columns', '0% 0% 100%');
    } else if (command === 'iris_full'){
        python = true
        javascript = true
        $('#editorspane').css('grid-template-columns', '49.75% 0.5% 49.75%');
    } else if (command == 'iris_run') {
        runPython()
        runJavascript()
    } else if (command == 'iris_run_py') {
        runPython()
    } else if (command == 'iris_run_js') {
        runJavascript()
    } 
    else if (command == 'js_clear') {
        $('#jsfile').html('index.js')
        editor1.setValue('')
    }
    else if (command == 'py_clear') {
        editor2.setValue('')
        $('#pyfile').html('index.py')
    }
     else if (command == 'iris_undo') {
        _undo()
    } else if (command == 'iris_redo') {
        _redo()
    } 
}
